/* 	This code shows real time implementation of the "Data Pipelining" Usage Model of the ADSP-2156x FIR/IIR Accelerators. 
	...
	- The code performs real time FIR/IIR processing on 12 channels of audio data. 7 channels are processed on the core and 5 channels on the accelerator.	
...
volatile unsigned int wp = FIR_TAPS;

static void Process_Block()
{
	int i;

	/* Process left channel on accelerator and right channel on core */
	/* Ping buffer */
	if(CallbackCount==1)
	{
		if(BypassProcessing==false)
		{
			...
			Block_Fixed_To_Float( (int*)&int_SP0ABuffer4, Input[8], Input[7], BLOCK_SIZE );

			TAKE_CYCLE_SNAPSHOT
			Process_FIR_IIR();
			TAKE_CYCLE_SNAPSHOT

		    Block_Float_To_Fixed( (int*)&int_SP0ABuffer1, Output[0], Output[7], BLOCK_SIZE  );
		}
		else
		{
			for(i=0;i<COUNT;i++)
	...
	if(CallbackCount==2)
	{
		if(BypassProcessing==false)
		{
			...
			Block_Fixed_To_Float( (int*)&int_SP0ABuffer5, Input[8], Input[7], BLOCK_SIZE );

			TAKE_CYCLE_SNAPSHOT
			Process_FIR_IIR();
			TAKE_CYCLE_SNAPSHOT

			Block_Float_To_Fixed( (int*)&int_SP0ABuffer2, Output[0], Output[7], BLOCK_SIZE  );
		}
		else
		{
			for(i=0;i<COUNT;i++)
